...
router.post("/", auth, jsonParser, async (req, res) => {
  try {
    ...
    bot.sendMessage(
      keys.bot.chatId,
      `Your order has been placed successfully! Order ID: ${orderDoc._id}`,
    );
...
router.get("/search", auth, async (req, res) => {
  try {
    ...
    if (ordersDoc.length > 0) {
      ...
      let orders = newOrders; // newOrders.map((o) => store.caculateTaxAmount(o));
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const ordersDoc = await Order.find(query)
      ...
      .populate("user", "id firstName lastName name phoneNumber")
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .exec();
    ...
    try {
      ...
      if (status === CART_ITEM_STATUS.cancelled) {
        ...
        if (cart.products.length === items.length) {
          // crucial data for analytics in admin dashboard
          // await Order.deleteOne({ _id: orderId });
